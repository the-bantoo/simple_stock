...
class StockIssue(StockController):
	def __init__(self, *args, **kwargs):
		super(StockIssue, self).__init__(*args, **kwargs)
	...
	def validate_purpose(self):
		...
		if self.job_card and self.purpose not in ["Material Transfer for Manufacture", "Repack"]:
			frappe.throw(
				_(
					"For job card {0}, you can only make the 'Material Transfer for Manufacture' type Stock Issue"
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in frappe.get_all(
				...
				frappe.delete_doc("Stock Issue", d.name)
	...
	def update_cost_in_project(self):
		...
		if self.project:
			amount = frappe.db.sql(
				...
				from
					`tabStock Issue` se, `tabStock Issue Detail` sed
				where
			...
			additional_costs = frappe.db.sql(
				...
				from
					`tabStock Issue` se, `tabLanded Cost Taxes and Charges` sed
				where
					se.docstatus = 1 and se.project = %s and sed.parent = se.name
					...
					if cstr(item.t_warehouse) == "":
						...
						transferred_materials = frappe.db.sql(
							"""
									select
										sum(qty) as qty
									from `tabStock Issue` se,`tabStock Issue Detail` sed
									where
	...
	def validate_fg_completed_qty(self):
		...
		for item_code, qty_list in item_wise_qty.items():
			total = flt(sum(qty_list), frappe.get_precision("Stock Issue Detail", "qty"))
	...
	def validate_difference_account(self):
		...
		for d in self.get("items"):
			if not d.expense_account:
				...
				frappe.throw(
					_(
						"Difference Account must be a Asset/Liability type account, since this Stock Issue is an Opening Entry"
					),
					OpeningEntryAccountError,
				)
		...
		for warehouse in ["s_warehouse", "t_warehouse"]:
			...
			for row in self.items:
				...
				for sn in get_serial_nos(row.serial_no):
					if sn in serial_nos:
						frappe.throw(
							_("The serial no {0} has added multiple times in the Stock Issue {1}").format(
	...
	def get_gl_entries(self, warehouse_account):
		gl_entries = super(StockIssue, self).get_gl_entries(warehouse_account)
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = frappe.get_doc("Stock Issue", self.outgoing_stock_entry)
	...
	def get_items(self):
		...
		if self.bom_no:
			...
			# fetch the serial_no of the first Stock Issue for the second Stock Issue
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			args = {
				"source_dt": "Stock Issue Detail",
				"target_field": "transferred_qty",
				"target_ref_field": "qty",
				"target_dt": "Stock Issue Detail",
				"join_field": "ste_detail",
				"target_parent_dt": "Stock Issue",
...
def move_sample_to_retention_warehouse(company, items):
	...
	stock_entry = frappe.new_doc("Stock Issue")
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = frappe.db.get_value("Stock Issue", source_name, "add_to_transit")
			if add_to_transit:
...
def get_used_alternative_items(
	...
	data = frappe.db.sql(
		...
		from
			`tabStock Issue` ste, `tabStock Issue Detail` sted
		where
...
def get_supplied_items(
	...
	fields = [
		"`tabStock Issue Detail`.`transfer_qty`",
		"`tabStock Issue`.`is_return`",
		f"`tabStock Issue Detail`.`{rm_detail_field}`",
		"`tabStock Issue Detail`.`item_code`",
	]

	filters = [
		["Stock Issue", "docstatus", "=", 1],
		["Stock Issue", subcontract_order_field, "=", subcontract_order],
	]

	supplied_item_details = {}
	for row in frappe.get_all("Stock Issue", fields=fields, filters=filters):
...
def get_stock_entry_data(work_order):
	stock_entry = frappe.qb.DocType("Stock Issue")
	stock_entry_detail = frappe.qb.DocType("Stock Issue Detail")
