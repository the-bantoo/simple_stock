...
frappe.ui.form.on('Stock Issue', {
	...
	refresh: function(frm) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					erpnext.utils.select_alternate_items({
						...
						child_doctype: "Stock Issue Detail",
		...
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(__('Received Stock Entries'), function() {
					...
					frappe.set_route('List', 'Stock Issue');
				}, __("View"));
			}
		}
		...
		if(frm.doc.docstatus==1 && frm.doc.purpose == "Material Receipt" && frm.get_sum('items', 			'sample_quantity')) {
			frm.add_custom_button(__('Create Sample Retention Stock Issue'), function () {
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					frappe.msgprint(__("Retention Stock Issue already created or Sample Quantity not provided"));
				}
			}
		});
	},
				...
				callback: function(r) {
					if (!r.message) {
						...
						$.each(r.message, function(i, item) {
							let d = frappe.model.add_child(cur_frm.doc, "Stock Issue Detail", "items");
...
frappe.ui.form.on('Stock Issue Detail', {
...
erpnext.stock.StockEntry = class StockEntry extends erpnext.stock.StockController {
	...
	clean_up() {
		// Clear Work Order record from locals, because it is updated via Stock Issue
