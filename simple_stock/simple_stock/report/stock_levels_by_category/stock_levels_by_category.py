...
def execute(filters: Optional[StockLevelsByCategoryFilter] = None):
	...
	#frappe.errprint("show_stock_ageing_data " + str(filters.get("show_stock_ageing_data")))
...
def get_columns(filters: StockLevelsByCategoryFilter):
	...
	if filters.get("show_stock_ageing_data") == 1:
		columns = [
			{
				"label": _("Category (Item Group)"),
...
def get_stock_ledger_entries(filters: StockLevelsByCategoryFilter, items: List[str]) -> List[SLEntry]:
	sle = frappe.qb.DocType("Stock Ledger Entry")
...
def get_item_warehouse_map(filters: StockLevelsByCategoryFilter, sle: List[SLEntry]):
	iwb_map = {}
...
def get_items(filters: StockLevelsByCategoryFilter) -> List[str]:
	...
	if item_code := filters.get("item_code"):
		...
		return frappe.get_all("Item", filters=item_filters, pluck="name", order_by="item_group")


def get_item_details(items: List[str], sle: List[SLEntry], filters: StockLevelsByCategoryFilter):
	item_details = {}
	if not items:
