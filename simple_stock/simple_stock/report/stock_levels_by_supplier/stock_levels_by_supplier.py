...
class StockLevelsByLocationSupplier(TypedDict):
	from_date: str
	to_date: str
...
def execute(filters: Optional[StockLevelsByLocationSupplier] = None):
	...
	for supplier in suppliers:
		...
		items = frappe.get_all("Item", 
			fields=['name'],
			filters=[
				["Item Default", "default_supplier", "=", supplier],
			],
			pluck="name"
		)
		#print(supplier, sup_itm)
		...
		report_data = {
			"warehouse": "",
			"currency": "",
			"item_code": "<strong>" + supplier + "</strong>",
			"company": "",
			"stock_oum": "",
			"bal_qty": ""
		}
		...
		report_data = {
			"warehouse": "",
			"currency": "",
			"item_code": "<strong>" + supplier + " Subtotal</strong>",
			"company": "",
			"stock_uom": "",
			"bal_qty": supplier_qty
		}
		data.append(report_data)
...
def get_columns(filters: StockLevelsByLocationSupplier):
	"""return columns"""
	columns = [
...
def get_stock_ledger_entries(filters: StockLevelsByLocationSupplier, items: List[str]) -> List[SLEntry]:
	sle = frappe.qb.DocType("Stock Ledger Entry")
...
def get_item_warehouse_map(filters: StockLevelsByLocationSupplier, sle: List[SLEntry]):
	iwb_map = {}
...
def get_items(filters: StockLevelsByLocationSupplier) -> List[str]:
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockLevelsByLocationSupplier):
	item_details = {}
	if not items:
