...
def get_item_suppliers():
	return frappe.get_all("Item Default", 
		fields = [ 'parent', 'default_supplier' ]
	)

def get_supplier(list, key):
	if key:
		for d in list:
			if key in d.parent:
				return d.default_supplier
	else:
		return ""
...
def execute(filters=None):
	...
	item_suppliers = get_item_suppliers()
	...
	for item in sorted(item_map):
		default_supplier = get_supplier(item_suppliers, item)
		data.append(
			[
				default_supplier,
				item,
